...
TC1: Creation Of User 2
    ...
    ${customer_2_id}=       get value from json     ${response.json()}      user.id
    ${customer_2_phone}=    get value from json     ${response.json()}      user.phone_number
        # Saving the data to the variables.json file
    set to dictionary       ${json_obj}             customer_2_id=${customer_2_id[0]}
    set to dictionary       ${json_obj}             customer_2_phone=${customer_2_phone[0]}
